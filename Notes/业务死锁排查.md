...
```
-- artistmgr.artist_manage_data definition
 
CREATE TABLE `artist_manage_data` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `user_id` int(11) DEFAULT NULL COMMENT '用户ID',
  `period_id` int(11) DEFAULT NULL COMMENT '周期ID',
  `manage_days` double(8,4) DEFAULT NULL COMMENT '管理人天',
  `manage_ratio` double(8,4) DEFAULT NULL COMMENT '管理工作占比',
  `created_at` timestamp NULL DEFAULT NULL COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `index_user_id` (`user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=8354 DEFAULT CHARSET=utf8mb4 COMMENT='管理人天的相关的数据';
```

在数据库表结构中，原有user_id和period_id两个普通索引。
...
![](https://raw.githubusercontent.com/lincentma/pic_bed/master/image/20220212233817.png)
